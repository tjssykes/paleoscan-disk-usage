...
def random_pale_color() -> str:
    """Return a random pale (light, low-saturation) colour as a '#rrggbb' hex
    string, suitable for use as a button background."""
...
class PaleoScanDiskUsageApp(tk.Tk):
    ...
    def _build_notebook(self):
        ...
        # -- Tab bar: a horizontally-scrollable strip of buttons (replaces
        # the old ttk.Notebook tabs). Each button gets a random pale colour.
        tab_bar_outer = ttk.Frame(container)
        tab_bar_outer.pack(side="top", fill="x")

        self._tab_canvas = tk.Canvas(tab_bar_outer, height=36, highlightthickness=0)
        self._tab_canvas.pack(side="top", fill="x")
        self._tab_hscroll = ttk.Scrollbar(
            tab_bar_outer, orient="horizontal", command=self._tab_canvas.xview
        )
        self._tab_hscroll.pack(side="top", fill="x")
        self._tab_canvas.configure(xscrollcommand=self._tab_hscroll.set)

        self._tab_button_frame = ttk.Frame(self._tab_canvas)
        self._tab_canvas.create_window((0, 0), window=self._tab_button_frame, anchor="nw")
        self._tab_button_frame.bind(
            "<Configure>",
            lambda e: self._tab_canvas.configure(scrollregion=self._tab_canvas.bbox("all")),
        )

        # -- Content area: pages are stacked on top of one another and we
        # raise the active one, instead of a notebook switching tabs.
        self.content_area = ttk.Frame(container)
        self.content_area.pack(side="top", fill="both", expand=True, pady=(6, 0))

        self.tabs: list[dict] = []  # [{"title": str, "frame": ttk.Frame, "button": tk.Button}, ...]
        ...
        self._add_tab_page(placeholder, "Info")

    # -- Button-tab helpers (stand in for what ttk.Notebook used to do) ------

    def _add_tab_page(self, frame, title: str, index: int | None = None) -> dict:
        """Add a page (a content frame + its own coloured button). If index
        is given, the button/page is inserted at that position instead of
        appended at the end."""
        btn = tk.Button(
            self._tab_button_frame,
            text=title,
            bg=random_pale_color(),
            activebackground=random_pale_color(),
            relief="raised",
            bd=2,
            padx=12,
            pady=5,
        ...
        tab_record = {"title": title, "frame": frame, "button": btn}

        if index is None or index >= len(self.tabs):
            self.tabs.append(tab_record)
        else:
            self.tabs.insert(index, tab_record)

        btn.configure(command=lambda t=tab_record: self._select_tab(t))
        self._repack_tab_buttons()

        frame.place(in_=self.content_area, x=0, y=0, relwidth=1, relheight=1)
        frame.lower()

        if self._active_tab is None:
            self._select_tab(tab_record)

        return tab_record

    def _repack_tab_buttons(self):
        for t in self.tabs:
            t["button"].pack_forget()
        for t in self.tabs:
            t["button"].pack(side="left", padx=(0, 4), pady=3)

    def _select_tab(self, tab_record: dict):
        for t in self.tabs:
            if t is tab_record:
                t["button"].configure(relief="sunken", bd=3)
            else:
                t["button"].configure(relief="raised", bd=2)
        tab_record["frame"].lift()
        self._active_tab = tab_record
    ...
    def _build_status_bar(self):
        ...
        self.progress.pack(side="left", padx=6, pady=3)

        ttk.Label(status, textvariable=self.status_var, anchor="w", padding=(6, 3)).pack(
            side="left", fill="x", expand=True
        )

        ttk.Label(status, text="v1.0:  Dr Tim J S Sykes (Calibrated Geoscience) 2026", anchor="e", padding=(6, 3)).pack(side="right")
    ...
    def _add_project_tab(self, path_str: str, analysis: dict):
        ...
        existing_titles = {t["title"] for t in self.tabs}
        ...
        frame = ttk.Frame(self.content_area, padding=8)
        self._add_tab_page(frame, tab_title)
    ...
    def _build_summary_tab(self):
        ...
        frame = ttk.Frame(self.content_area, padding=8)
        self._add_tab_page(frame, "Summary", index=0)
